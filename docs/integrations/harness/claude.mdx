...
If the host supplies and manages a compatible Claude Agent SDK, install
`nemo-fabric[claude-min]`. This extra still installs the NeMo Fabric runtime and
...
`nemo-fabric-adapters-claude`. The standalone distribution contains only
adapter-owned runtime dependencies and requires a compatible Claude Agent SDK
